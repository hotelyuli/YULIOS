...
using (id = public.current_hotel_id() and public.current_role() = 'owner')
with check (id = public.current_hotel_id() and public.current_role() = 'owner');

create policy profiles_select_self_or_management
on public.profiles for select to authenticated
using (
  hotel_id = public.current_hotel_id()
  and (id = auth.uid() or public.current_role() in ('owner', 'manager'))
);
...
using (hotel_id = public.current_hotel_id() and public.current_role() = 'owner')
with check (hotel_id = public.current_hotel_id() and public.current_role() = 'owner');
